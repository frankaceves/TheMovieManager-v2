...
extension TMDBClient {
    ...
    */
    ...
    func getFavoriteMovies(_ completionHandlerForFavMovies: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        ...
        // ORIGINAL MUTABLE METHOD: AccountIDFavoriteMovies = "/account/{id}/favorite/movies"
        ...
        //print("updated mutable Favorites Method: \(mutableMethod)")
        ...
        let _ = taskForGETMethod(mutableMethod, parameters: parameters as [String:AnyObject]) { (results, error) in
            ...
            if let error = error {
                ...
                if let results = results?[TMDBClient.JSONResponseKeys.MovieResults] as? [[String:AnyObject]] {
                    ...
                    //print("get fav movies results: \(movies)")
    ...
    func getWatchlistMovies(_ completionHandlerForWatchlist: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        ...
        // ORIGINAL MUTABLE METHOD: AccountIDWatchlistMovies = "/account/{id}/watchlist/movies"
        mutableMethod = substituteKeyInMethod(mutableMethod, key: TMDBClient.URLKeys.UserID, value: String(TMDBClient.sharedInstance().userID!))!
        //print("updated mutable Watchlist Method: \(mutableMethod)")
        /* 2. Make the request */
        let _ = taskForGETMethod(mutableMethod, parameters: parameters as [String: AnyObject]) { (results, error) in
    ...
    func getRatedMovies(_ completionHanderForRatedMovies: @escaping (_ result: [TMDBMovie]?, _ error: NSError?) -> Void) {
        /* 1. Specify parameters, method (if has {key}), and HTTP body (if POST) */
        let parameters = [TMDBClient.ParameterKeys.SessionID: TMDBClient.sharedInstance().sessionID!]
        var mutableMethod: String = Methods.AccountRatedMovies
        // ORIGINAL MUTABLE METHOD: AccountIDFavoriteMovies = "/account/{id}/rated/movies"
        mutableMethod = substituteKeyInMethod(mutableMethod, key: TMDBClient.URLKeys.UserID, value: String(TMDBClient.sharedInstance().userID!))!
        /* 2. Make the request */
        let _ = taskForGETMethod(mutableMethod, parameters: parameters as [String: AnyObject]) { (results, error) in
            /* 3. Send the desired value(s) to completion handler */
            if let error = error {
                completionHanderForRatedMovies(nil, error)
            } else {
                //need results > rating as Int?
                if let results = results?[TMDBClient.JSONResponseKeys.MovieResults] as? [[String: AnyObject]] {
                    let movies = TMDBMovie.moviesFromResults(results)
                    print("rated movies successful")
                    completionHanderForRatedMovies(movies, nil)
                } else {
                    completionHanderForRatedMovies(nil, NSError(domain: "getRatedMovies parsing", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse rated movies"]))
                }
            }
        }
