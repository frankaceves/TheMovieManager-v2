...
class MovieDetailViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if let movie = movie {
            ...
            //apply getRatedMovies
            
            TMDBClient.sharedInstance().getRatedMovies({ (movies, error) in
                if let movies = movies {
                    
                    for movie in movies {
                        //if movie ID's match, apply rated movie data to movie
                        if movie.id == self.movie!.id {
                            self.movie = movie
                            //check for rating
                            if movie.rating == self.movie!.rating {
                                let starRating = Double(movie.rating)! / 2
                                self.navigationItem.title?.append(" Rating: \(starRating) Stars")
                            }
                        }
                    }
                }
